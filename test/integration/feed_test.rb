...
class FeedTest < ActionDispatch::IntegrationTest
  test 'should see microposts from subscribed platforms' do
    user1 = User.create!(username: 'u1', email: 'u1@example.com', password: '1234567', confirmed_at: DateTime.now)
    user2 = User.create!(username: 'u2', email: 'u2@example.com', password: '1234567', confirmed_at: DateTime.now)
    plat = platforms(:plat)
